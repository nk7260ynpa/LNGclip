...
- 頻道名稱、訂閱數：從 `/videos` 分頁的 `channel`、`channel_follower_count` 取得
- 影片數：分別抓取 `/videos` 與 `/shorts` 兩個分頁的影片數量再加總
...
with yt_dlp.YoutubeDL(opts) as ydl:
    ...
    # info["playlist_count"] → 一般影片數

    shorts = ydl.extract_info(f"{channel_url}/shorts", download=False)
    # shorts["playlist_count"] → Shorts 影片數
    # 影片總數 = videos + shorts
```
